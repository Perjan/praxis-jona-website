...
The collector supports only Umami's current API contract. It uses expanded path metrics, event series, and event visitor statistics, and reports unique visitors for the primary conversion event. Do not add compatibility logic for older Umami APIs; an API mismatch must fail visibly so the scheduled operator can repair the current integration deliberately.
