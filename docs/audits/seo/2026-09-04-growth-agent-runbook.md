...
The deployed server currently exposes Umami's current API contract. The collector detects this with a read-only path-metrics probe, uses expanded path metrics plus event series and event visitor statistics, and reports unique visitors for the primary conversion event. It retains a legacy-v2 fallback for compatible older servers. When a server cannot expose unique visitors per custom event, event-per-visitor rates must remain `null`/“unavailable”; never convert missing data to zero. Pageview-based CTA rates remain measurable.
