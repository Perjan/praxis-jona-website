...
- The deployed current Umami API exposes unique visitors for the primary custom event through event visitor statistics. The collector supports this current contract only and must fail visibly on an API mismatch; compatibility logic for older Umami APIs is explicitly out of scope.
...
| 4 | Shipped | Fix duplicated PRP hub lead; add/validate visible FAQ schema and service schema | Eng/Clinical | One H1/lead, valid markup generated from visible FAQs, unchanged medical copy |
| 5 | Shipped | Map query overlap between iron cost and infusion pages; rewrite internal anchors | SEO/Eng | Exact iron anchors favor cost page; generic infusion anchors favor hub |
...
| 7 | In progress (audit complete; owner edits pending) | Audit GBP, Doctolib, Jameda, Doctify and top directories; fix NAP discrepancy | Local | Wrong Das Örtliche phone and directory-hours gaps verified; Jameda completion pending |
| 8 | In review | Clinical research/brief for iron effect/timing article | Clinical/Content | Evidence dossier complete; claims, timeline language and red flags await Dr. Gjolli's approval |
...
- **2026-09-15 — Local visibility and citation audit:** live mobile Maps research found Praxis Jona #1 near the storefront for `eiseninfusion berlin` but #7 for `prp behandlung berlin`. A 3×3 PRP grid found the clinic in only one of nine full top-20 result sets. The claimed profile has 20 reviews versus PRP leader ORVĒA's 75, and no owner replies appeared in Praxis Jona's latest 20-review sample. Citation checks verified a wrong public phone number on Das Örtliche, hours mismatches on Doctolib and Arzt-Auskunft, and a sparse Jameda profile. No external listing was edited; owner/profile access is needed for the corrections. The durable OpenSEO project and aggregate-only audit prevent future runs from repurchasing or retaining reviewer-level data.
- **2026-09-16 — Iron effect/timing research dossier:** completed the Day-8 evidence and clinician-review handoff for the planned `/blog/eiseninfusion-wirkung-wann` article. The source set includes current guidelines, a 21-trial meta-analysis, positive and negative placebo-controlled trials, German diagnostic guidance and EMA product-safety material. The editorial position separates laboratory response from symptom response, rejects a universal day-count promise and records explicit claim boundaries, red flags, counterevidence, internal links and measurement gates. No public medical copy or claim was changed; publication remains blocked on Dr. med. Jonida Gjolli's claim-by-claim approval, practice-specific product/follow-up details and actual review date.
- **2026-09-17 — Day-5 anchor audit and first fix:** audited internal anchors between the iron cost page and the vitamin-infusion hub. The iron page already points to the hub with a generic "Infusionstherapie" secondary CTA (correct direction). The hub page had no contextual link back to the iron page at all and separately listed "Eisenmangel" as one of its own treatment indications with no disambiguation, which is the actual query-overlap risk. Added minimal inline-link support (`[text](url)`) to the shared `LongevityMarkdownPage` renderer, which previously rendered all body links as literal bracket text, and linked the hub's "Eisenmangel" bullet to `/leistungen/eiseninfusion-kosten`. Verified with `tsc --noEmit`, a production build, a rendered DOM check of the live anchor, and the existing 8 `tests/seo` vitest cases (all pass). No medical copy changed. Remaining day-5 scope: a fuller query-overlap map across other iron/PRP touchpoints beyond this one page pair.
- **2026-09-17 — Day-5 anchor fix extended to the PRP cluster:** ported the same `[text](url)` inline-link support into `AestheticMarkdownPage`'s shared list/paragraph renderer (previously bracket syntax rendered as literal text there too) and linked the PRP hub's "und dunklen Augenringen" bullet to `/aesthetik/prp-behandlung/prp-augenregion-bei-dunklen-augenringen`, the specialized page that owns the "PRP Augenringe Berlin" query. Checked the separate price-link gap the audit flagged: no PRP-cluster body copy mentions cost, but the hub, both PRP sub-pages (`prp-gesicht`, `prp-augenregion-bei-dunklen-augenringen`) and the hair page (`/leistungen/haarausfall-berlin-mitte`) already render a prominent "Preise ansehen" CTA button to `/aesthetik/preise` via the shared `CtaButtons` component, confirmed in rendered production HTML — so no new link was added there. Verified with `tsc --noEmit`, the existing 8 `tests/seo` vitest cases (all pass), a production build and a rendered-HTML check that the new anchor and existing price CTAs render as real `<a href>` tags. No medical copy changed.
- **2026-09-17 — Day-5 anchor fix extended to the PRP hair cluster:** the `AestheticMarkdownPage` renderer already supports `[text](url)` links (same code path as the PRP aesthetic fix above, since the hair section shares the component), so no code change was needed. Linked the hair page's ("hair" section, Mikronährstofftherapie & Infusionstherapie subsection) "Eisenmangel" bullet to `/leistungen/eiseninfusion-kosten`, matching the already-shipped iron-cluster pattern. Left two other bare "Eisenmangel" mentions and a "Ferritin," bullet in the same section unlinked to avoid over-optimizing the same exact-match anchor on one page. Verified with `tsc --noEmit`, the existing 8 `tests/seo` vitest cases (all pass), a production build and a rendered-HTML check on `/leistungen/haarausfall-berlin-mitte` confirming the anchor renders as a real `<a href="/leistungen/eiseninfusion-kosten">Eisenmangel</a>` tag. No medical copy changed.
- **2026-09-17 — Day-23 breadcrumb/cluster-link audit:** checked whether `BreadcrumbList` JSON-LD and cluster internal links already exist across every current iron/PRP page, ahead of marking day 23 further along. Confirmed via source: `LongevityMarkdownPage` generates `breadcrumbSchema` for both the iron cost page and the infusion hub; `AestheticMarkdownPage` generates it for the PRP hub and every PRP detail page (`prp-gesicht`, `prp-augenregion-bei-dunklen-augenringen`, hair); and `app/components/pricing/pricingSchema.ts` generates it for `/aesthetik/preise`. Breadcrumbs and the current cluster's contextual links (day 5's three fixes) are therefore already in place for every *existing* priority URL. The day-23 success signal ("every new asset has 3+ relevant incoming internal links") is about the not-yet-published iron/PRP articles from days 9–21, so the remaining scope is blocked on those, not on anything actionable today. No code changed.
- **2026-09-17 — Day-25 recrawl tooling built:** `next-sitemap.config.js` previously stamped every URL with the build timestamp as `lastmod`, so all 188 URLs carried an identical, meaningless "now". Added a `transform` that resolves each URL's real last-changed date from git history of the content that actually backs it (contentlayer post file for blog/guide posts, the shared markdown/component source for cluster pages, the route's own `page.tsx` as a fallback, or no `lastmod` at all when nothing resolves) — verified locally: 188 URLs collapsed from 1 identical timestamp to 14 distinct real dates. Added `scripts/seo/submit-recrawl.mjs`, wired into `postbuild`: it resubmits the sitemap to the Search Console API (reusing the existing `GSC_ACCESS_TOKEN`/gcloud auth from `scripts/gsc/`) and pings the IndexNow API (Bing/Yandex/Seznam) for every sitemap URL, appending a result line to `docs/audits/seo/recrawl-submissions.log`. Deliberately does **not** use Google's Indexing API, which is restricted to JobPosting/BroadcastEvent content and risks a manual action if used for ordinary pages. The script only makes real network calls when `VERCEL_ENV=production` (i.e. an actual production deploy) or `FORCE_SEO_SUBMIT=1`; a local `npm run build` (used to verify this) or a preview deployment just logs a skip line, so iterating locally never spams the live APIs. Added the required IndexNow key file at `public/6656db04d230f588226ead1a5cfa59fa.txt`. Verified with `tsc --noEmit`, `npm test -- tests/seo` (8/8 pass) and a full local `npm run build`.
...
| 2026-09-14 | PRP hub and shared German aesthetic-template technical SEO | Shipped | [merged PR #13](https://github.com/Perjan/praxis-jona-website/pull/13) | No medical copy changed | Remove responsive duplicate hero copy; generate FAQ schema from visible source copy; focused regression tests added first |
| 2026-09-15 | Local Maps visibility and citation consistency | Audited; corrections pending | [Local SEO audit](./2026-09-15-praxis-jona-local-seo-audit.md) · [PR #13](https://github.com/Perjan/praxis-jona-website/pull/13) | Owner access required for external edits | Iron Maps rank #1; PRP center rank #7 and only 1/9 grid points found; recheck 2026-10-13 after corrections |
| 2026-09-16 | Iron effect/timing cornerstone research | In review | [Clinical research dossier](./2026-09-16-eiseninfusion-wirkung-research-dossier.md) · [PR #13](https://github.com/Perjan/praxis-jona-website/pull/13) | Dr. med. Jonida Gjolli approval pending | One canonical timing/effect owner; no public claim published; measure weekly after eventual launch |
...
| 2026-09-16 | Answer iron-effect timing with one evidence-led article that separates laboratory and symptom response | Existing query demand is fragmented across synonymous timing searches, while guidelines and trials do not support a guaranteed day-count. One canonical owner prevents thin duplicates and supports a defensible answer | After Dr. Gjolli approves the claim set and supplies practice-specific product, follow-up, red-flag and review-date details |
